...
class User():
	...
	def reset_loging_attempts(self, addition):
		self.increment_reading -= addition
class Privleges():
	def __init__(self, can_add_post = "can add post", can_delete_post = "can delete post",
				can_ban_user = "can ban user"):

		self.can_add_post = can_add_post 
		self.can_delete_post = can_delete_post
		self.can_ban_user = can_ban_user

	def look_privileges(self):
		print("This App can " + self.can_add_post + ', ' + self.can_delete_post
			+ ' and ' + self.can_ban_user + " post.")
...
print()
describe_user.allcorrect.look_privileges()
